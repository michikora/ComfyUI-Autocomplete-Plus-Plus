...
function resolveLinkedNumber(promptGraph, value, visited = new Set()) {
    ...
    if (Array.isArray(value) && value.length >= 1) {
        ...
        if (targetNode && targetNode.inputs) {
            // If the source node is in explicit random mode (e.g. backend seed generators), do not lock onto dummy static values
            const mode = String(targetNode.inputs.mode || targetNode.inputs.action || "").toLowerCase();
            if (mode === "random" || mode === "randomize") {
                return null;
            }
...
export function getCanvasGroundTruth() {
    const canvasGroundTruth = new Map();
    if (typeof app !== "undefined" && app.graph && Array.isArray(app.graph._nodes)) {
...
export function processPromptPayload(payload, canvasGroundTruth = null) {
    ...
    if (!canvasGroundTruth) {
        canvasGroundTruth = getCanvasGroundTruth();
    }
            ...
            try {
                if (
                    ...
                    if (options && options.method === "POST" && typeof options.body === "string") {
                        ...
                        const canvasGroundTruth = getCanvasGroundTruth();
                        const processed = processPromptPayload(parsed, canvasGroundTruth);
