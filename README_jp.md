...
- **シード値連動・順次選択・前回抽選結果の保持**:
    ...
    - **PNG 読み込み時の抽選結果復元（実験的機能）**: **Keep Last Choice** モードと本機能が有効な状態で、生成情報を含む PNG 画像を ComfyUI にドラッグ＆ドロップまたは読み込んだ際、元画像で実際に選ばれていたダイナミックプロンプトやワイルドカードの抽選結果を自動的に復元します。マスターシード値を変更しない限り、復元された組み合わせがそのまま維持され、意図しない再抽選を防ぎます。
        - _動作モード_: サードパーティ製プラグインによる読み込みに対応するため、本機能はブラウザのネットワーク応答（`fetch`）を監視します。この際、開発者ツール（DevTools）の呼び出し履歴（コールスタック）において無関係な通信の発生元が本拡張機能経由として表示され、デバッグ時の通信元特定に影響を与える場合があります（通常のリクエスト内容や通信データの改変は行わず、無効化時は元の動作に戻ります）。インポート時のみ一時的に監視を行いデバッグへの影響を抑える `Lite`（デフォルト）と、互換性を最優先して常時監視する `Full` を選択できます。日常的にフロントエンドの開発や通信の切り分けを行う場合は、本機能を無効のまま運用することを推奨します。
...
- **波括弧の自動クローズ**:
    - `{` を入力すると自動的に閉じ括弧 `}` が挿入され、カーソルが括弧の間に配置されます。
    - テキストを選択した状態で `{` を入力すると、選択範囲が波括弧で囲まれます。
    - 閉じ括弧 `}` の手前で `}` を入力した場合は、重複挿入せずにカーソルが括弧の外へ移動します。
    - 空の `{}` の間で `Backspace` を押すと、左右の括弧が同時に削除されます。
...
- **`Autocomplete++ Formatting Controller`**:
    - Anima 絵師モード（`@`）、カンマ自動挿入、アンダースコアをスペースへ置換、丸括弧エスケープ、波括弧の自動クローズ、フォーカスが外れた時の自動整形、カンマ後のスペース追加、文末および行末のカンマ削除、アンダースコア置換、およびアンダースコア維持タグの指定（全体設定に追加、または完全上書き）などの設定をワークフロー単位で上書きします。
...
3. **Formatting**:
    - `Anima Artist '@' Prefix Mode`: 絵師タグの `@` プレフィックス処理。
    ...
    - `Auto-Close Curly Braces`: `{` 入力時に閉じ括弧 `}` を自動挿入。
...
8. **Wildcards & Dynamic Prompts**:
    ...
    - `Restore Choices on PNG Import`: PNG 読み込み時の抽選結果復元機能のオンオフ（実験的機能、デフォルト: 無効）。
    - `PNG Import Interception Mode`: PNG 読み込み時の動作モード（`Lite`: インポート時のみ一時監視 / `Full`: 常時監視）。
